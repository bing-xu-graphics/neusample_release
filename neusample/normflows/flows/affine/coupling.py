...
def positional_encoding(tensor, basis_num):
    value = 0.0
    encodingsin = [tensor]
    encodingcos = []
    for j in range(basis_num):
        value = 2**j * tensor 
        encodingsin.append(torch.sin(value))
        encodingcos.append(torch.cos(value))

    if len(encodingsin) == 1:
      return encodingsin[0]
    else:
        return torch.cat(encodingsin+encodingcos, dim=-1)

# def positional_encoding(
#     tensor, num_encoding_functions=6, include_input=True, log_sampling=False
# ) -> torch.Tensor:
...
class AffineCoupling(Flow):
    ...
    def forward(self, z):
        ...
        param = self.param_map(encoded_z1)
        if self.scale:
            shift = param[:, 0::2, ...]
            scale_ = param[:, 1::2, ...]
...
class AffineCouplingBlock(Flow):
    ...
    def forward(self, z):
        ...
        for flow in self.flows:
            ...
            log_det_tot += log_det
        ###TODO check
        # logdet = torch.sum(torch.log(torch.abs(1-(torch.tanh(z))**2) + 1e-5), -1)        
        # z = torch.tanh(z).clone()
        # log_det_tot = log_det_tot + logdet
        #######################
    ...
    def inverse(self, z):
        ...
        ############################
        # logdet = torch.sum(torch.log(torch.abs(1/(1-z*z+1e-5))+1e-5), -1)
        # z  = 0.5*torch.log(torch.abs((1+z)/(1-z+1e-5)) + 1e-5)
