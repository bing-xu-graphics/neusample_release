...
class Coupling(Flow):
    ...
    def forward(self, inputs, context=None, encode = None):
        ...
        if(encode):
            transform_params = self.transform_net(encoding.positional_encoding_1(identity_split, encode, log_sampling=False), context)
        else:
    ...
    def inverse(self, inputs, context=None,encode = None):
        ...
        if(encode):
            ...
            encoded_half_input = encoding.positional_encoding_1(identity_split, encode, log_sampling=False)
            # print(encoded_half_input.shape)
            transform_params = self.transform_net(encoded_half_input, context)
        else:
            transform_params = self.transform_net(identity_split, context)
