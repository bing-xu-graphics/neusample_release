...
class DiagGaussian(BaseDistribution):
    ...
    def forward(self, cond_vec = None, num_samples=1):
        eps = torch.randn(
    ...
    def log_prob(self, z, cond_vec = None):
        if self.temperature is None:
            log_scale = self.log_scale
        else:
            log_scale = self.log_scale + np.log(self.temperature)
...
class MultiUniform(BaseDistribution):
    def __init__(self, ndim, low, high):
    ...
    def forward(self, cond_vec=None, num_samples=1, randseed = None):
        z = self.sample(cond_vec, num_samples, randseed)
        pdf = torch.zeros(z.shape[0], device=DEVICE_, dtype=DTYPE_)
        return z, pdf

    def sample(self, cond_vec=None, num_samples=1, randseed = None):
    ...
    def log_prob(self, z, cond_vec=None):
        return torch.zeros(z.shape[0], device=DEVICE_, dtype=DTYPE_)
...
class GaussianPCACond(BaseDistribution):
    """
    Gaussian distribution resulting from linearly mapping a normal distributed latent
    variable describing the "content of the target"
    """
