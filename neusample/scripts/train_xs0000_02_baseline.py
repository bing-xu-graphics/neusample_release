...
exp_name = "xs0000_02_shell_baseline"
...
def main():
    ...
    # model = GMMWeightedCondLarge(n_modes=1, dim=2, cond_C=30, trainable=True)
    model = GMMWeightedCondIsoLarge(n_modes=1, dim=2, cond_C=30, trainable=True)
    ...
    if TRAIN:
        ...
        for idx_step in range(num_steps):
            ...
            for iter, sampleBatch in enumerate(it_dataset):
                ...
                ll= model.log_prob(x, cond_vec, wo)
                loss = - ll.mean()
