...
from model.base2d import Base2D
...
class ModelTrainer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        ret = self.base2d(wi[...,:2].reshape(B*S,2),condition_feature.repeat_interleave(S,0),vq=True)
        pdf_est = ret['pdf']
        cos_theta = ret['cos_theta'].reshape(B,S,-1)[:,0]
        pdf_est = pdf_est.reshape(B,S)
        loss = NF.mse_loss(btf,pdf_est)

        # vq loss
        vq_theta = ret['vq_theta'].reshape(B,S,-1)[:,0]
        vq_pdf = ret['vq_pdf'].reshape(B,S)

        loss_vq = NF.mse_loss(btf,vq_pdf)\
                + NF.mse_loss(vq_theta,cos_theta)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for b in range(math.ceil(self.lobe_size*self.lobe_size*1.0/batch_size)):
            ...
            logq = self.base2d(wi[0,b0:b1],condition_feature.repeat_interleave((b1-b0),0))['pdf']
